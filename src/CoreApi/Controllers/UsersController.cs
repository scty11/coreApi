...
namespace CoreApi.Controllers
{
    ...
    {
        private IRepositories.IUserRepository _userRepository;
        private IRepositories.IScheduleRepository _scheduleRepository;
        private IRepositories.IAttendeeRepository _attendeeRepository;

        int page = 1;
        int pageSize = 10;

        public UsersController(IRepositories.IUserRepository userRepository,
            IRepositories.IScheduleRepository scheduleRepository,
            IRepositories.IAttendeeRepository attendeeRepository)
        {
        ...
        {
            IEnumerable<Schedule.Model.Schedule> _userSchedules = _scheduleRepository.FindBy(s => s.CreatorId == id);

            if (_userSchedules != null)
            {
                IEnumerable<ScheduleViewModel> _userSchedulesVM =
                    Mapper.Map<IEnumerable<Schedule.Model.Schedule>, IEnumerable<ScheduleViewModel>>(_userSchedules);
        ...
        public IActionResult Delete(int id)
        {
            ...
            else
            {
                ...
                IEnumerable<Schedule.Model.Schedule> _schedules = _scheduleRepository.FindBy(s => s.CreatorId == id);
